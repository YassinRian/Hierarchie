WITH 
CHILD_LEVEL AS 
(SELECT DIM_KDR_KPL_ORG_HIER_ID, KOSTENDRAGER_KIND, ORGANISATIE_BK_VADER, ORGANISATIE_KIND, ORGANISATIE_BK_KIND, HR_NIVEAU FROM [OCW_EBS_1FA].DIM_KDR_KPL_ORG_HIER WHERE KOSTENDRAGER_KIND LIKE 'B6%'),
BUDGET AS (
SELECT DISTINCT 
  DIM_DATUM_JAAR_ID
, DIM_DIENST_ID
...
FROM [OCW_EBS_1FA].FCT_BUDGETMUTATIEREGELS FBUD
LEFT OUTER JOIN [OCW_EBS_1FA].DIM_PSB_STATUS PSBSTAT ON FBUD.DIM_PSB_STATUS_ID = PSBSTAT.DIM_PSB_STATUS_ID 
WHERE PSBSTAT.DIENSTCONTROL_AKKOORD = 'Ja'
AND DIM_DIENST_ID = 65 
),
HIERARCHIE AS (
SELECT 
...
, TO_NUMBER('') AS LVL6
, '' AS ORGANISATIE_LVL6
FROM [OCW_EBS_1FA].DIM_KDR_KPL_ORG_HIER LVL0
...
FROM [OCW_EBS_1FA].FCT_BALANSREGELS_PSB FPSB 
INNER JOIN [OCW_EBS_1FA].DIM_GROOTBOEKPERIODE GBP ON FPSB.DIM_GROOTBOEKPERIODE_ID = GBP.DIM_GROOTBOEKPERIODE_ID
WHERE FPSB.DIM_DIENST_ID = 65 
) QY0
) QY1
