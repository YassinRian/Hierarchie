WITH 
CHILD_LEVEL AS 
(SELECT DIM_KDR_KPL_ORG_HIER_ID, KOSTENPLAATS_KIND, ORGANISATIE_BK_VADER, ORGANISATIE_KIND, ORGANISATIE_BK_KIND, HR_NIVEAU FROM DMT.DIM_KDR_KPL_ORG_HIER WHERE KOSTENDRAGER_KIND LIKE 'B6%'),
BUDGET AS (
SELECT 
  DIM_DATUM_JAAR_ID
, DIM_DIENST_ID
...
FROM DMT.FCT_BUDGETMUTATIEREGELS FBUD
LEFT OUTER JOIN DMT.DIM_PSB_STATUS PSBSTAT ON FBUD.DIM_PSB_STATUS_ID = PSBSTAT.DIM_PSB_STATUS_ID 
...
,INITCAP(LOWER(TO_CHAR(QY2.GBP_EINDDATUM_PERIODE, 'MON'))) as MAAND_NAAM
,INITCAP(LOWER(TO_CHAR(QY2.GBP_EINDDATUM_PERIODE, 'DD-MON'))) as GBP_DD_MON
,QY2.FPSB_DIM_DIENST
